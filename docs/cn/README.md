...
[术语表](references/glossary.md)、[常见问题](references/faq.md)和[错误排查](references/troubleshooting.md)不单独列入学习路径。建议在以下时机翻阅：
...
每条路径走完后，可以用下面的标准检验自己：

| 完成路径 | 你应该能做到 |
|---------|---------|
| 用户路径 ⭐⭐ | 独立用 `KronosPredictor.predict()` 跑出预测结果 |
| 微调路径 ⭐⭐⭐ | 在自有数据上完成一轮微调，loss 稳定下降 |
| 原理路径 ⭐⭐⭐⭐ | 解释 BSQ 量化与层级令牌的设计动机，并用源码佐证 |

### 常见误区

- Kronos 输出的是 OHLCV 预测值，不是买卖信号——需要你自行根据预测结果制定交易策略。
- mini 的上下文长度是 2048（其他模型为 512），处理长序列时反而需要更多显存。选型别光看参数量，优先考虑上下文需求。详见 [模型对比](advanced/07-model-comparison.md)。
- 预测时 `KronosPredictor` 内部自动调用分词器，但微调时需要单独训练。给 mini 配了 `Tokenizer-base` 会导致结果异常。
- 不同模型的 `context_length` 和分词器搭配不同，直接套配置会出错。参数差异见上方速查表。

---

## 文档原则

这些文档遵循以下约定：参数量、上下文长度、接口签名和默认值等事实性内容以源码和根目录 `README.md` 为准；需要解释设计动机时，会写成”从实现可以看出”或”这是一种合理解释”，不把未公开论证的内容写成结论；示例代码复用仓库已有接口和路径；行号引用标注具体位置（如 `kronos.py:270`），方便对照源码；跨文档使用统一的中文译法，首次出现时标注英文原文。
