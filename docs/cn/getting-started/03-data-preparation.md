...
**为什么使用实例级标准化？** 这是 `KronosPredictor.predict()` 和 `predict_batch()` 在源码中的默认做法：每条输入序列先按自身均值与标准差做归一化，再用 `clip` 做裁剪。这样做的直接好处是不同价格量级的数据都能被映射到相近的数值范围内。从统计学角度看，正态分布中 99.99994% 的值落在 ±5σ 范围内。clip=5 等价于保留几乎所有正常波动的数据，同时过滤掉极端异常值。
