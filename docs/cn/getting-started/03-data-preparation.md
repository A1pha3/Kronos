...
**为什么使用实例级标准化？** 这是 `KronosPredictor.predict()` 和 `predict_batch()` 在源码中的默认做法：每条输入序列先按自身均值与标准差做归一化，再用 `clip` 做裁剪。这样做的直接好处是不同价格量级的数据都能被映射到相近的数值范围内。这里不再把 `clip=5` 解释为某种统一适用的统计结论，因为仓库没有给出这部分实验说明。
