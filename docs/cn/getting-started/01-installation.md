...
**A**: PyTorch 需要根据你的操作系统和 CUDA 版本选择正确的安装包。访问 [PyTorch 官网](https://pytorch.org/get-started/locally/) 获取适合你系统的安装命令，先单独安装 PyTorch，再安装其他依赖。
...
**A**: Apple Silicon Mac（M1/M2/M3/M4）支持 MPS 后端。KronosPredictor 会自动检测并使用 MPS 设备。Intel Mac 只能使用 CPU。
