...
Kronos 是首个面向金融 K 线数据的开源基础模型，被 AAAI 2026 接收。它的核心思路是**将 K 线预测转化为"语言建模"问题**——先把连续的 OHLCV 数据"翻译"为离散令牌（类似于文字），再用 Transformer 模型预测未来的令牌序列。
...
Kronos 并非唯一的金融预测模型。了解它与其他方法的定位差异有助于判断它是否适合你的需求：

| 模型类型 | 代表 | 与 Kronos 的区别 |
|---------|------|----------------|
| 传统量化模型 | ARIMA、GARCH | 统计建模，参数少但需要手动指定模型结构 |
| 深度学习时序模型 | LSTM、Temporal Fusion Transformer | 端到端回归，针对单一任务训练 |
| 通用时序基础模型 | TimesFM、Chronos、Moirai | 面向通用时间序列（温度、流量等），不专门针对金融 K 线 |
| **金融 K 线基础模型** | **Kronos** | **专为金融蜡烛图设计，OHLCV 结构感知，层级令牌捕捉价格形态** |

**Kronos 的独特定位**：它是一个**专门为金融 K 线数据**设计的基础模型。与通用时序模型不同，Kronos 的输入维度（6 维 OHLCV）、令牌化方式（BSQ 量化）、层级解码（s1/s2 条件依赖）都是针对 K 线数据的特征量身定制的。这意味着在金融 K 线预测任务上，Kronos 通常比通用时序模型更有效，但不适合非金融的时序预测任务（如天气预报、传感器数据等）。

**你需要记住的唯一核心概念**：Kronos 不是直接预测"明天收盘价是多少"，而是预测"明天的 K 线属于哪一种令牌模式"。这种设计让预测结果更稳定，也允许你通过调整采样参数获得不同风格的结果。
...
> 注意：首次导入时不会自动下载模型。模型会在你调用 `from_pretrained()` 时自动从 HuggingFace Hub 下载。
