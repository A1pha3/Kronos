...
这篇文档对比四个 Kronos 模型的规格差异，并给出选型建议：
...
> 参数量从 4.1M 到 499.2M 跨越约 120 倍。更大的模型通常有更强的表示能力，但这不等于"任何数据上都更准"。文件大小、显存占用、推理速度受硬件、框架版本和输入长度影响，因此不在表中给出固定数值。
...
- `d_model`（模型维度）是影响模型表达力的最关键参数——更大的 `d_model` 意味着更强的表示能力，但需要更多训练数据才能充分发挥
- `n_layers` 影响模型捕捉长距离依赖的能力。层数越多，模型能"记住"越远的历史模式，推理时间也近似线性增长
- 推理时的显存占用主要由 `d_model`、`n_layers` 和 `seq_len` 共同决定
