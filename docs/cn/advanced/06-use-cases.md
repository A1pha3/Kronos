...
以下内容展示 Kronos 在不同场景下的配置方式和使用建议：

- [ ] 能识别 Kronos 适用的预测场景及其局限性
- [ ] 能根据不同市场和数据类型选择合适的模型配置
...
Kronos 的预测结果应当作为多个决策参考之一，而非唯一依据。重点关注趋势方向和波动范围——相比追求精确数值，预测的涨跌方向通常更有参考价值。如果条件允许，用多次采样生成置信区间（参见场景 2），并将 Kronos 输出与传统技术指标、基本面分析交叉比对。
