...
这篇文档展示 Kronos 在不同场景下的配置方式和使用建议：
...
Kronos 在 45+ 个全球交易所的数据上进行了预训练，理论上支持任何金融市场的 K 线预测。
...
# 后续使用方式完全相同
pred_df = predictor.predict(df=x_df, x_timestamp=x_timestamp, y_timestamp=y_timestamp, pred_len=60)
```
...
Kronos 的预测结果应当作为多个决策参考之一，而非唯一依据。重点关注趋势方向和波动范围——相比追求精确数值，涨跌方向的判断通常更有参考价值。如果条件允许，用多次采样生成置信区间（参见场景 2），并将 Kronos 输出与传统技术指标、基本面分析交叉比对。
...
**关键原则**：Kronos 提供的是"如果历史模式延续，未来可能如何"的概率性参考，不等于确定性预测。实际交易决策还需要考虑风险偏好、仓位管理、止损策略等因素。
...
在使用 Kronos 的过程中，有几个容易被误解的观念需要澄清：

### 误区 1：模型集成一定优于单模型

集成学习在传统机器学习中往往能显著提升效果，但在 Kronos 场景下并非如此。不同规模的 Kronos 模型（small、base、large）共享相同的预训练数据，只是参数容量不同。这意味着它们的误差来源高度相关——面对同样的"困难样本"，所有模型都可能犯错。

实际测试表明，在主流市场（A 股、美股日线）上，多模型集成对 MSE 的改善通常仅为 5%-15%，方向准确率的提升更为有限。如果追求更稳健的结果，增加单模型的采样次数（`sample_count`）往往比组合多个模型更经济有效。

真正能从集成中获益的场景是：结合 Kronos 预测与传统技术指标（如均线、MACD、RSI）进行多源信号验证，因为这些方法的底层逻辑完全不同，具有真正的独立性。

### 误区 2：多粒度交叉验证能消除不确定性

多时间粒度交叉验证（如日线判断趋势 + 分钟线寻找入场点）能提高信号的可信度，但并不能消除预测的不确定性。这种方法本质上是在多个视角下检验同一个市场，如果底层驱动因素发生了模型无法捕捉的变化（如突发政策调整、流动性危机），所有粒度的信号都可能同时失效。

正确的理解是：多粒度交叉验证**降低了忽略重要信号的概率**（因为不同粒度反映不同层级的市场参与者行为），但模型本身的概率性局限依然存在。当多个粒度的信号一致时，可以提升对当前判断的信心；当信号冲突时，应更加谨慎而非认为"一定能找到正确答案"。

### 误区 3：Kronos 的置信区间等价于统计学的置信区间

Kronos 通过多次采样（如 `sample_count=10` 或生成多条独立路径）计算 p5/p95 分位数来构造"置信区间"，但这与经典统计学中的置信区间有本质区别：

- **统计学置信区间**：基于明确的概率分布假设（如正态分布）和抽样理论，95% 置信区间意味着在重复抽样下，约 95% 的区间会包含真实参数值。
- **Kronos 的经验区间**：仅反映模型自身在不同随机采样下的输出分散程度。它衡量的是"模型预测的一致性"，而非"覆盖真实值的概率"。

例如，如果模型的底层假设存在偏差（如市场机制发生了结构性变化），即使经验区间很窄，真实值也可能完全落在区间之外。因此，Kronos 的经验区间更适合用于**横向比较**（同一标的在不同时间点的预测不确定性），而非作为严格的概率保证。
...
- [ ] 能说出至少 3 种 Kronos 适合的使用场景
- [ ] 知道 Kronos 不适合用于高频交易的原因
- [ ] 能根据市场类型选择合适的预测参数
- [ ] 能解释多次采样生成置信区间的方法
- [ ] 理解 Kronos 预测结果"仅供研究参考"的含义
