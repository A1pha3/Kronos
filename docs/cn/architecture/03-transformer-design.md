...
以下内容分析 Kronos Transformer 各组件的设计决策与替代方案：

- [ ] 能解释 Kronos 选择 Pre-Norm + RMSNorm + RoPE + SwiGLU 的理由及各自替代方案
- [ ] 能说明因果掩码和填充掩码在自注意力和交叉注意力中的不同行为
- [ ] 能理解时间嵌入的分离编码+求和策略的设计权衡
- [ ] 能评估"替换某个组件"对整体系统的影响
...
class MultiHeadAttentionWithRoPE(nn.Module):
    def forward(self, x, key_padding_mask=None):
        q = q_proj(x).reshape(B, S, H, D).transpose(1, 2)   # 源码使用 .view()
        k = k_proj(x).reshape(B, S, H, D).transpose(1, 2)   # 源码使用 .view()
        v = v_proj(x).reshape(B, S, H, D).transpose(1, 2)   # 源码使用 .view()

        q, k = self.rotary(q, k)  # 应用 RoPE

        # 注：实际源码会将 key_padding_mask 转换为扩展的 attn_mask（见下文填充掩码部分），
        # 此处伪代码简化为直接传入 padding_mask
...
> **精确性说明**：上述伪代码中 `.reshape()` 用于直观展示形状变换，实际源码（`model/module.py`）使用 `.view()` 实现。两者功能等价——`.view()` 要求张量在内存中连续，`.reshape()` 在不连续时会自动复制。由于投影层输出始终连续，两者行为一致。
...
**RoPE 外推的实际限制**：虽然 RoPE 理论上支持长度外推，但实践中外推距离过远时注意力权重的精度会显著下降。Kronos 的预训练模型在 max_context=512（Kronos-mini 为 2048）下训练，如果推理时使用远超训练长度的上下文，预测质量会衰减。建议外推范围不超过训练长度的 1.5-2 倍——如果需要在更长序列上使用，应增大训练时的 `max_context` 或使用微调适配。
...
class FeedForward(nn.Module):
    def __init__(self, d_model, ff_dim, dropout=0.0):  # 注：源码中参数名为 ffn_dropout_p
...
**非因果模式的性能影响**：推理时使用非因果交叉注意力意味着 `F.scaled_dot_product_attention` 可以使用更高效的注意力实现路径。PyTorch 的 SDPA 在 `is_causal=False` 时可以利用 Flash Attention 等优化，而 `is_causal=True` 需要额外应用因果掩码。实际上，`is_causal=False` 并不排斥 Flash Attention——PyTorch 的 SDPA 后端在无因果掩码时同样会自动选择 Flash Attention 或 Memory-Efficient Attention 路径（只要输入满足其对齐要求）。虽然对于 `DependencyAwareLayer` 中 query 长度为 1 的场景，这种差异微乎其微，但在语义上，非因果模式更准确地反映了推理时的信息结构——s2 的预测应当充分利用所有已知上下文。
...
总体而言，Kronos 的模型规模（已开源最大：d_model=832, n_layers=12）远小于典型 LLM（LLaMA-7B 的 d_model=4096, n_layers=32），因此上述性能差异在实际使用中几乎不可感知。
...
加载预训练模型，输入两段相同数据但不同时间偏移的时间戳，对比嵌入层的余弦相似度。
...
with torch.no_grad():
    # tokenizer.encode() 返回的是 (s1_ids, s2_ids) 元组
    indices = tokenizer.encode(x, half=True)
    emb1 = model.embedding(indices) + model.time_emb(stamp1)
    emb2 = model.embedding(indices) + model.time_emb(stamp2)
...
**调试提示**：如果相似度远低于 1.0，检查 (1) `stamp1` 和 `stamp2` 的第 5 列（month）是否超出了嵌入表的范围（0-12）；(2) 模型是否使用了可学习时间嵌入（`learn_te=True`），此时不同时间位置可能学到了不同的表示。

### 练习 2：替换激活函数并观察收敛差异

在 `model/module.py` 中找到 `FeedForward` 类，将 SwiGLU 替换为标准 ReLU：
...
**注意**：替换后需要从零训练（不能加载预训练权重），因为权重矩阵的语义已改变（`w3` 不再使用）。

**验证方法**：对比替换前后的训练曲线——ReLU 版本的收敛速度通常更慢，最终损失可能更高。如果差异很小，说明在小规模模型上 SwiGLU 的优势不显著。

---

## 自测清单

- [ ] 能说出 Kronos Transformer 与 LLaMA 在组件选择上的共同点
- [ ] 能解释为什么交叉注意力在推理时使用非因果模式
- [ ] 能说明 SwiGLU 相比 ReLU 的优势与代价
- [ ] 能列出将 Pre-Norm 替换为 Post-Norm 时需要注意的事项
- [ ] 能解释时间嵌入中"分离编码+求和"优于"拼接+线性变换"的理由
