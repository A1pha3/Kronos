...
这篇从系统全局视角分析 Kronos 的模块组成与数据流。读完应能做到：
...
│   │   │   │      → 输出形状 (B, 1, d_model)
│   │   │   └─ DualHead.proj_s2 → s2_logits (B, 1, 1024)
...
- [ ] 能解释为什么 Kronos 使用"令牌化 + 自回归"而非直接回归
- [ ] 能说出实例级标准化的优势和代价
- [ ] 能解释滑动窗口在推理中如何管理超长历史
- [ ] 能在源码中快速定位任意模块的实现位置
- [ ] 能评估修改某个组件对系统其他部分的影响
...
基于当前仓库实现，以下几条观察适合作为二次开发时的检查清单：

1. **接口层与模型层解耦**：`KronosPredictor` 负责数据清洗、标准化、时间特征与推理编排，`Kronos` 和 `KronosTokenizer` 负责核心建模逻辑。你可以替换上层数据流程，不必直接改模型主体。

2. **层级令牌会牵动多处定义**：`s1_bits` / `s2_bits` 不只是模型超参数，还会影响嵌入表、分类头、分词器量化维度与权重兼容性。改动时不能只改一处。

3. **实例级标准化是接口契约的一部分**：当前预测入口在内部统一执行均值、标准差与裁剪逻辑。改预处理策略时，需要同步评估预测入口、批量入口和示例脚本的一致性。

4. **Hub 集成是模型类的基础能力**：`PyTorchModelHubMixin` 直接挂在模型类上，本地路径加载与 HuggingFace Hub 加载共享同一套接口。

5. **分词器训练态与推理态路径不同**：`forward()`、`encode()`、`decode()` 各自承担不同职责。做源码扩展时不能只验证训练路径而忽略推理路径。

6. **KronosPredictor 不是 nn.Module**：它是普通 Python 类，通过持有 `self.model` 和 `self.tokenizer` 的引用来编排推理流程。不会被 `model.state_dict()` 序列化，也不参与梯度计算。自定义推理管线时可以直接实例化并传入修改后的模型对象。构造函数会自动将模型移至检测到的设备——如果你手动做了设备管理，需要留意这一点。
...
| 步骤 | 占比 | 说明 |
|------|------|------|
| 分词器编码 (encode) | ~15-20% | 3 层编码器（n_enc_layers=4，实际 Transformer 块为 3）+ BSQ 量化 |
| 自回归推理 (120步) | ~50-60% | 每步都需要完整的 Transformer 前向传播 |
| 分词器解码 (decode) | ~15-20% | 3 层解码器重建 OHLCV |
