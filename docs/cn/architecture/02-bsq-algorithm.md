...
向量量化的目标是将一个连续向量 **z** ∈ R^D 映射到有限的码本 C = {c_1, c_2, ..., c_K} 中。传统方法（如 VQ-VAE）面临两个问题：码本利用率低（大部分条目从未被使用，即"死码"问题）和高维灾难（D 较大时码本需要指数级增长）。

BSQ（Binary Spherical Quantization）将量化问题转化为**二值化问题**：每个维度量化为 +1 或 -1，码本大小为 2^D，且**天然均匀分布**，从根本上避免死码。
...
**analytical 模式的数学推导**：直观上，sigmoid 在这里充当一个"软二值化阈值"——它将连续的归一化向量分量映射为 0 到 1 之间的概率值，表示该维度取 +1 的可能性。与硬二值化（直接取 sign）不同，sigmoid 是可导的，使得梯度可以流过量化决策，从而支持端到端训练。以下推导说明为什么 BSQ 的二值码本结构让这个概率恰好退化为一个 sigmoid 表达式：
...
| 场景 | 推荐调整 | 调整方向 | 判断标准 |
|------|---------|---------|---------|
| 码本利用率低于 10% | 增大 `γ₀` 和 `γ` | 各增加 0.1 | 观察 `metrics["used_codes"]` 唯一码字数 |
| 重建损失居高不下 | 减小 `β` | 减小 0.01 | commit loss 与重建损失的比值 |
| 训练早期不稳定 | 增大 `inv_temperature` | 设为 2-5 | 待训练稳定后逐渐降回 1 |
| 训练后期码本塌缩 | 增大 `ζ` | 增加 0.01 | 观察码本熵是否持续上升 |

**典型调试流程**：
1. 先用默认参数训练，观察码本利用率和重建损失
2. 如果码本利用率低（< 10%），先调 `γ₀`，再调 `γ`
3. 如果重建质量差，先检查数据预处理，再考虑减小 `β`
4. `ζ` 通常不需要调整——它是熵正则化的总权重，与 `γ₀`/`γ` 联动
...
```
D = 20, group_size = 4（Kronos 实际使用的值）
→ 5 个子组，每个子组 4 位
→ 每个子组的码本大小：2^4 = 16
```
...
#### s1_bits / s2_bits 比例的实际影响

`codebook_dim` 的总分配保持不变时，s1 与 s2 的比例决定了"粗粒度"与"细粒度"信息的分配：

| 配置 | s1 词汇量 | s2 词汇量 | 效果分析 |
|------|----------|----------|---------|
| 10 / 10（默认） | 1024 | 1024 | s1 和 s2 均衡分配，每层有 10 位精度 |
| 12 / 8 | 4096 | 256 | s1 捕获更丰富的粗粒度模式，s2 仅做少量修正 |
| 8 / 12 | 256 | 4096 | s1 区分力弱，s2 承担主要精细化工作 |
| 6 / 14 | 64 | 16384 | s1 极度粗略，s2 压力过大——不推荐 |

**选择原则**：

- **数据变化模式简单的市场**（如低波动率蓝筹股日线）：可以尝试增大 s1_bits、减小 s2_bits，因为粗分类已足以区分大部分 K 线形态
- **数据变化丰富的市场**（如高波动率小盘股分钟线）：建议保持 s1_bits >= s2_bits，确保粗分类有足够的区分力
- **推理效率考虑**：s2 的解码依赖 s1 的采样结果（通过 `DependencyAwareLayer`），s1 词汇量越大，推理时的候选空间越大，但计算开销增加有限（因为 s1 是独立采样的）

> **经验法则**：除非有充分的实验证据，否则建议保持默认的 10/10 均衡分配。Kronos 论文中的实验也基于此配置。
...
理解 BSQ 的量化过程后，以下是量化结果如何在 Kronos 预测流水线中被使用的完整链路：

```
输入 OHLCV → 编码器 → L2 归一化 → BSQ 二值化 → 切分为 s1/s2 索引
                                                       ↓
                        自回归 Transformer 预测 s1 → 采样 → DependencyAwareLayer
                                                                           ↓
                                                    预测 s2 → 采样 → 拼接 s1+s2
                                                                           ↓
                                              BSQ 解码（二值码 → 连续向量）→ 解码器 → OHLCV
```

**关键观察**：

- **训练时**：BSQ 的 STE 和损失函数确保编码器学会产生"容易量化"的表示
- **推理时**：BSQ 的损失函数不再参与——量化操作本身（L2 归一化 → 二值化 → 缩放）是确定性的，不需要梯度。Transformer 只需要预测正确的 s1/s2 索引，然后通过 BSQ 解码器重建连续向量
- **编码器和解码器协同**：BSQ 的 commit loss 使编码器输出靠近量化点，这意味着即使量化操作丢弃了信息，编码器也会学会将重要信息编码到"不会被量化破坏"的维度上

这种"训练时学习适应量化，推理时直接使用量化"的设计，是 BSQ 在 Kronos 中高效工作的根本原因。
...
# 此处使用小端序 [1,2,4,8,...]，与 BSQuantizer.bits_to_indices() 一致
index = ((zhat + 1) / 2 * torch.tensor([1,2,4,8,16,32,64,128], dtype=torch.float)).sum(dim=-1).long()
print(f"索引: {index.item()}")
```

> **关于端序的重要说明**：Kronos 源码中存在两种不同的索引计算函数，使用不同的字节序：
>
> | 函数 | 所在类 | 端序 | basis 生成方式 | D=8 时的 basis |
> |------|--------|------|----------------|----------------|
> | `codes_to_indexes()` | `BinarySphericalQuantizer` | **大端序** | `2 ** torch.arange(D-1, -1, -1)` | `[128, 64, 32, 16, 8, 4, 2, 1]` |
> | `bits_to_indices()` | `BSQuantizer` | **小端序** | `2 ** torch.arange(0, D, 1)` | `[1, 2, 4, 8, 16, 32, 64, 128]` |
>
> 两者的区别在于：同一个二值向量 `[1, 0, 1, 0]` 在大端序下被解释为二进制 `1010`（十进制 10），在小端序下被解释为二进制 `0101`（从右往左读，十进制 5）。**索引数值不同，但排序是一一对应的**——两种端序产生的索引排列顺序互为位反转（bit-reversal）关系。
>
> 在实际使用中：
> - `bits_to_indices()`（小端序）用于 Kronos 的令牌化流程（`BSQuantizer.forward` 中调用），是 s1/s2 令牌索引的最终来源
> - `codes_to_indexes()`（大端序）用于 `BinarySphericalQuantizer` 内部的指标收集（如统计码本利用率），不影响最终输出
>
> 本练习使用 `bits_to_indices()` 的小端序，因为它对应 Kronos 实际输出的令牌索引。
