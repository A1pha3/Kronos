...
**从信息论角度理解**：在信息论中，一个离散随机变量的最优编码长度等于其熵。如果一次预测 20 比特的完整码本，模型需要在 2^20 个类别上学习一个概率分布——这在数据有限时会导致大多数类别的概率估计接近零，只有少数类别有足够的训练样本支撑。将 20 比特拆分为两个独立的 10 比特预测后，每个子任务只需要在 2^10 = 1024 个类别上建立概率分布，每个类别能获得更多的训练样本，概率估计更稳定。代价是，两次独立预测的组合无法精确表达两个子码本之间的相关性——但在 s2 条件依赖 s1 的架构下（通过 `DependencyAwareLayer`），这种相关性被部分恢复了。
