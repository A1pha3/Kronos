...
Kronos 的解决方案是**分层**：将 20 比特的码本拆分为两个 10 比特的子码本。模型先预测 s1（1,024 种选择），再在 s1 的约束下预测 s2（1,024 种选择），将 1,048,576 种组合的预测分解为两次 1,024 选 1 的决策。
...
BSQ 量化器将 20 维连续向量量化为 20 维二值向量 `{-1, +1}^20`。这 20 个比特被分为两部分：
...
- **保持均衡（默认 10/10）**：通常是稳妥的起点。两级的预测难度相当（均为 1,024 分类），训练更稳定。
...
- 仅用 s1 的重建误差应显著低于仅用 s2，说明 s1（粗粒度）承载了大部分主要信息。
- 仅用 s2 的重建误差最大，因为 s2 编码的是残差细节，缺少粗粒度基准则信息量有限。
- s1+s2 的重建误差最小，体现了两级令牌协同工作的优势。
