...
单一令牌面临一个困境：码本太小则表达能力不足，码本太大则预测难度急剧增加。Kronos 的解决方案是**分层**——将 20 比特的码本拆分为两个 10 比特的子码本（默认配置 `s1_bits=10, s2_bits=10`）。模型先预测 s1（1,024 种选择），再在 s1 的约束下预测 s2（1,024 种选择），将 1,048,576 种组合的预测分解为两次 1,024 选 1 的决策。
