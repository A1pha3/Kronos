...
大语言模型（如 GPT）直接处理离散的文本令牌，但金融 K 线数据是连续的多维数值。直接在连续空间做自回归预测有两个核心问题：误差累积（每步的微小偏差会在多步预测中被放大）和无法利用成熟的令牌采样策略（温度、top-k、top-p）。KronosTokenizer 的做法是**先将连续值"翻译"为离散令牌，再在离散空间进行预测**。

打个比方：这类似图像压缩中的"有损压缩"——KronosTokenizer 将高精度的 OHLCV 数据压缩为有限的几种令牌（码本），丢失了部分精度，但保留了最重要的特征。
...
**直觉理解**：传统 VQ 像是在一个"图书馆"里找最接近的书（需要搜索），BSQ 则是判断每个维度"偏正还是偏负"（只需要看符号）。后者精度略低，但计算高效且不会遇到码本利用率问题。
...
- [ ] 为什么 `forward()` 返回两个重建结果 `z_pre` 和 `z`，而 `decode()` 只返回一个？
- [ ] `post_quant_embed_pre` 和 `post_quant_embed` 的输入维度有什么差异？原因是什么？
- [ ] 共享解码器的设计是怎样的——同一组 `self.decoder` 层如何被 `z_pre` 和 `z` 各遍历一次？
- [ ] `half=True` 在 `encode()` 和 `decode()` 中分别做了什么？
- [ ] BSQ 量化包含哪些关键步骤（L2 归一化、二值化 + STE、缩放、索引计算）？其中 STE 解决什么问题？
