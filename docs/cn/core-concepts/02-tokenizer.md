...
大语言模型（如 GPT）直接处理离散的文本令牌，但金融 K 线数据是连续的多维数值。直接在连续空间做自回归预测会遇到两个问题：误差累积（每步的微小偏差会在多步预测中被放大）和无法利用成熟的令牌采样策略（温度、top-k、top-p）。KronosTokenizer 的做法是**先将连续值"翻译"为离散令牌，再在离散空间进行预测**。
